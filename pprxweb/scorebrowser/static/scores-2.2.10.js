...
const mfcPoints = [0,  0.1,  0.25,  0.25,  0.5,  0.5,  0.5,   1,   1,   1,   1,   2,   4,   6,   8,  15,  25,  25,  25,  25]
const sdpPoints = [0, 0.01, 0.025, 0.025, 0.05, 0.05, 0.05, 0.1, 0.1, 0.1, 0.1, 0.2, 0.4, 0.6, 0.8, 1.5, 2.5, 2.5, 2.5, 2.5]
...
$(document).ready(function () {
	...
	reqsPromise = fetch("/static/ranks-2.2.11.json").then(x => x.json())
		...
		columns: [
			...
			// 13:
			{
				...
				render: function(data, type, row, meta) {
					...
					targetPoints = Math.floor(baseFlarePoints[data.rating] * (1 + (0.06001 * targetGauge)))
	...
	function computeFlareSummary() {
		...
		scoresTable.rows().every(function() {
			...
			if (data.flare_gauge !== null) {
				points = Math.floor(baseFlarePoints[data.rating] * (1 + (0.06001 * data.flare_gauge)))
			}
